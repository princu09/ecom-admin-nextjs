...
const Index = () => {
  ...
  const handleDelete = async (data) => {
    if (
      confirm(`Are You Sure ? You Want to Delete ${data.name} Subcategory ?`) ==
      true
    ) {
      await axios
        .delete(`${process.env.NEXT_PUBLIC_BACKEND_URL}/subcategory`, {
          id: data._id,
        })
        .then(() => getData());
    }
  ...
  const getData = async () => {
    axios.get(`${process.env.NEXT_PUBLIC_BACKEND_URL}/subcategory`).then((res) => {
  ...
  return (
    <div>
      <div className="bg-accentShadow p-3 rounded-[3px] text-accentColor w-full flex items-center justify-between mb-5 font-medium">
        <p className="font-semibold">All Subcategory</p>
        <button
          ...
          onClick={() => Router.push("/admin/subcategory/add-subcategory")}
        >
          <FaPlus /> ADD
        </button>
      </div>
